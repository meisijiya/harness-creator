...
- **承接方是具名的工程 skill，按场合二选一**：系统提示中带 `superpowers` 引导词时，需求对齐、规格与计划、实现与测试、代码审查、分支收尾由它承担；无该引导词时全部由 `mattpocock` 承担。**两种场合下，状态与阻塞边 → `to-tickets`、会话交接 → `handoff` 都归 `mattpocock`**，共同前置是一次性运行 `/setup-matt-pocock-skills` 配置 tracker。本技能**不代为发起或执行**，也不为其产物指定位置。
...
3. 优先最小化：仅当用户的问题确实涉及跨会话记忆、工具安全、多代理协调、基准测试或 harness 维护时，才加载对应参考并加产物。→ 输出：要加载的参考（可为空）
...
| 4 | 更新 | 目标仓路径 → 候选改动清单 + 需你判断清单，🔴 获批后落地候选改动 |
...
状态与交接产物**不在交付清单内**：由承接方产生、自行决定位置。无法创建文件时，改为提供确切的文件内容与命令。
