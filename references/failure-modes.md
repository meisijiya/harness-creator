...
| 走到 🔴 CHECKPOINT，但运行环境无提问工具（或"信息已足够、可自行推断"） | **停在原地**：不得以推断或"知情后继续"替代批准；把待确认问题写出来（回复正文，或任务材料落点 `.scratch/` 下的一处），**不写盘、不交付半成品** | 仍无法交互时，按最小假设写明"假设 + 待确认项"但不落盘；用户答复后再执行 |

## 补充原则

- **两档行事，别把闸门当提醒**：一般异常「先告知再执行」——说明发生了什么、将采取什么动作，然后继续。**🔴 CHECKPOINT 不同：必须先获批**，「我已经能推断出来」不是跳过它的理由（要确认的正是这个推断），「知情后继续」也不构成批准。运行环境无提问工具时停在原地把问题写出来，不写盘、不交付半成品。
