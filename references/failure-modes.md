...
| 模式探测信号矛盾或缺失 | 按信号重判：`docs/agents/*` / `CONTEXT.md` / `docs/adr/` / 工单系统——`.scratch/` **不算**信号（第三方 skill 也会写它）；**第三方工具的存在（上游教学、调研等 skill）同样不构成模式信号** | 转「第一步」第 2 步的 🔴 CHECKPOINT（询问仓库用途），不自行断言 |
