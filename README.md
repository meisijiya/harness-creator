...
**更新与优化是显式调用的一轮**：用户要求更新或优化 harness 时走「更新」任务——先列**本次会话产出**里可能让 harness 失准的点，再用体检得分与 FAIL 行作交叉证据（最低分只是候选瓶颈，先确认因果；并列最低全部列出），列「将改 / 将删」清单走 🔴 CHECKPOINT 获批后落地，改完复验。流程见 [`references/harness-maintenance-pattern.md`](references/harness-maintenance-pattern.md)。
