...
- **[mattpocock/skills](https://github.com/mattpocock/skills)**——**唯一的强相关伙伴**。本技能与之的分工是硬边界：状态与阻塞边（`to-tickets`）、会话交接（`handoff`）、tracker 配置（`/setup-matt-pocock-skills`）、蓝图定调时的追问（`grilling`，模型可调用；`grill-me` 是用户调用，按边界不代发起）全部由它承担，本技能**不再复制**任何一份仓内台账。
