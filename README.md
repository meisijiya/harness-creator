...
**状态与生命周期不在表里，这是刻意的。** 上游把两者算作 harness 子系统，本技能的做法是**委派**：状态与阻塞边交给 `to-tickets`，会话交接交给 `handoff`（写到操作系统临时目录，不落本仓）。因此本技能既不落这两类产物，也不为它们指定位置；评估器同样不给它们打分——否则一个委派正确的仓库会被判成坏仓库。
...
状态与交接产物**不在其中**：由 `to-tickets` / `handoff` 产生，且交接不落本仓。
