...
`superpowers` 的在场判据是**运行期观察**：官方形态把 `using-superpowers` 引导词注入会话上下文，代理看自己的提示词即可判断，不必检查任何文件。若本机只有第三方移植版（无 hook），该引导词不存在 → 按降级规则归 `mattpocock`，这是**正确行为而非缺陷**。
...
| 范围边界 | 生成的指令文件必须**具名**写出承接方，并**按场合在两家之间路由**：在场判据（`superpowers` 引导词）存在 → 工程阶段归它；不存在 → 全归 `mattpocock`；而 `to-tickets`、`handoff`、`/setup-matt-pocock-skills` **两种场合都在**——"某个工程 skill 管"不可证伪且让代理无处可查。路由那半的检测是**逐词**的：删掉任一关键词都必须点名它缺失（`routing`）且证明检测器没有失聪（`routingTeeth`），一个种子违规样本证明不了后者。**且**不得携带已移除的治理学说（仓内台账、落点、产物追踪策略、受控放行、模式开关、分册）。两向都要断言：只禁不立会被空文件满足，只立不禁会被"继续派活"的模板满足；检测器同样用一份故意违规的样本自证有牙。注意 `.scratch/`、`docs/agents/`、`ADR`、`CONTEXT.md` **不再属于违规**——它们是承接方自己的产物，点名它们正是委派句要干的事。禁项 `owner-installation check` 只禁**安装检查**（是否已安装、提示安装），不禁**运行期在场判断**——后者正是路由键 |
