...
`run-benchmark.mjs` 在此之上先跑一遍**工具链自检**：**14 项断言**（12 个独立检查 + 脚手架打分 + 双语打分），外加 eval 覆盖计分；任一项 FAIL 都会让脚本以退出码 1 结束。这些关卡守的是技能自己的不变量：
...
| 收尾 / 整理扫描器安全 | 唯一一个「以删除为目的」看产物的脚本：必须**只读**；无 `--session-ref` 时只许标「无法判定」而非「陈旧」；给了 ref 仍须能标出历史（抑制不得是永久的）；收尾须抑制 prune 而整理仍须产出候选；`done` 无 evidence 在任何模式下都不得进删除候选 |
...
- [x] HTML 评估报告
- [x] 结构性基准报告
- [x] 评估用例 19 条（含 tracker 模式集成）；`run-benchmark` 的 eval 覆盖检查 10 项（控制台的 `(10/10)` 指覆盖检查，不是用例数）
...
- [x] 工具链自检 14 项断言（每项附反证）：脚本可跑、双语打分、字节上限、产物护栏（含 tracker 骨架）、模式闸门、`--dry-run` 一致性、自引用可达性、审计瓶颈并列、空项目门禁、交接识别、蓝图槽、条目模板克制、指令文件不变量、收尾／整理扫描器安全
